...
// MSB-first, MODE0, conservative 4 MHz
const SPISettings _currentSPI = SPISettings(8000000L, MSBFIRST, SPI_MODE0);
...
void _writeSPI(uint8_t headerLength, uint8_t* headerBuffer, uint8_t bodyLength, uint8_t* bodyBuffer) {
  ...
  for (int i = 0; i < bodyLength; i++) {
    SPI.transfer(bodyBuffer ? bodyBuffer[i] : 0x00); // write values
  }
  delayMicroseconds(5);
...
void _readSPI(uint8_t headerLength, uint8_t* headerBuffer, uint8_t readLength, uint8_t* readBuffer) {
  ...
  for (int i = 0; i < readLength; i++) {
    readBuffer[i] = SPI.transfer(JUNK); // read values (MSB-first, no reverse)
  }
  delayMicroseconds(5);
  digitalWrite(SPI_CSn, HIGH);
  SPI.endTransaction();
...
void DW3000_readData(uint8_t reg, uint8_t offset, uint8_t len, uint8_t* data) {
  ...
  switch (offset) {
    case JUNK:
      // Short Addressed Transaction (no sub-address)
      header[0] = ADDR_8 | (reg << 1) | READ;
      break;

    default:
      // Full Addressed Transaction (with sub-address)
      header[0] = ADDR_16 | (reg << 1) | READ; // sub-address present flag implied by ADDR_16 macro in this driver
      header[1] = offset;                      // send offset directly (no shift/reverse)
      header_len++;
      break;
  }
...
void DW3000_writeData(uint8_t reg, uint8_t offset, uint8_t len, uint8_t* data, SPI_WRITE_TYPE wtype) {
  ...
  switch (wtype) {
    case FAST:
      // Short Addressed Transaction (fast command)
      header[0] = ADDR_8 | (reg << 1) | FAST_CMD | WRITE;
      break;

    case SHORT:
      // Short Addressed Transaction (no sub-address)
      header[0] = ADDR_8 | (reg << 1) | WRITE;
      break;
    ...
    case FULL:
      // Full Addressed Transaction (with sub-address)
      header[0] = ADDR_16 | (reg << 1) | WRITE; // sub-address present
      header[1] = offset;                       // direct (no shift/reverse)
      header_len++;
      // keep original M0/M1 behavior if your code uses it
      if (wtype == MASK_01) { header[1] |= M0; }
      if (wtype == MASK_10) { header[1] |= M1; }
      if (wtype == MASK_11) { header[1] |= (M1 | M0); }
      break;
  }

  // Write body as-is (no bit reverse)
...
void DW3000_getDevID() {
  ...
  Serial.print("RIDTAG : ");Serial.println(ridtag);
}
...
void DW3000_getOUI() {
  ...
  Serial.print("OUI : "); Serial.println(oui);
}
...
uint32_t DW3000_getSysStatus() {
  uint32_t sysStatus = 0;               // (unchanged as requested)
  uint8_t data[4] = {0, };
